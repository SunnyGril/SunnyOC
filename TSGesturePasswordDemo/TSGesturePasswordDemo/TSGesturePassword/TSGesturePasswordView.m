...
#import "TSGesturePasswordView.h"
...
@interface TSGesturePasswordView()
...
@implementation TSGesturePasswordView
...
- (void) initView {
    ...
    for (int i=0; i<BUTTON_COUNT; i++) {
        TSGesturePasswordButton *button = [TSGesturePasswordButton buttonWithType:UIButtonTypeCustom];
...
- (void)layoutSubviews {
    ...
    for (int i=0; i<self.subviews.count; i++) {
        TSGesturePasswordButton *button = self.subviews[i];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    // 检测哪个按钮被点中了
    for (TSGesturePasswordButton *button in self.subviews) {
...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    // 合成轨迹序列
    for (TSGesturePasswordButton *button in self.selectedButtons) {
        // 清除选中状态
        button.selected = NO;
    ...
    // 调用代理方法
    if ([self.delegate respondsToSelector:@selector(endGestureLockView:didFinishedWithPath:)]) {
        [self.delegate endGestureLockView:self didFinishedWithPath:passPath];
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i=0; i<self.selectedButtons.count; i++) {
        TSGesturePasswordButton *button = self.selectedButtons[i];
